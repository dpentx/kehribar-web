...
export const GET: APIRoute = async () => {
  ...
  const [blogPosts, nixPosts] = await Promise.all([
    getCollection('blog', (p) => p.id.startsWith('tr/')),
    getCollection('nixzone', (p) => p.id.startsWith('tr/')),
  ]);

  const blogEntries = blogPosts.map((p) => ({
    title: p.data.title ?? p.id.split('/').pop()?.replace(/\.md$/, '') ?? p.id,
    url: langPath(lang, `/blog/${p.id.slice('tr/'.length).replace(/\.md$/, '')}`),
  }));

  const nixEntries = nixPosts.map((p) => ({
    title: p.data.title ?? p.id.split('/').pop()?.replace(/\.md$/, '') ?? p.id,
    url: langPath(lang, `/nixzone/${p.id.slice('tr/'.length).replace(/\.md$/, '')}`),
  }));
